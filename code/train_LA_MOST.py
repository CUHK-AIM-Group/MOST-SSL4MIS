...
from dataloaders.datasets_3d import StrongWeakLA_PancreasStyle as StrongWeakLAHeart
from dataloaders.masking import Masking, DepthWiseMasking, VoxelWiseMasking, CenterMasking
...
parser.add_argument('--exp', type=str,  default='MOST', help='exp_name')
...
parser.add_argument('--labelnum', type=int,  default=4, help='trained samples', choices=[4, 8, 16])
...
# mask
parser.add_argument('--mask_type', type=str,  default='mask', choices=['mask', 'depthwise_mask', 'voxelwise_mask', 'none', 'center_mask'])
...
# lr schedule
parser.add_argument('--lr_schedule', type=str, default='multistep', choices=['cosine', 'multistep'])
...
parser.add_argument('--bcp_weight', type=float, default=1.0, help='loss weight for copy pasted data')
...
snapshot_path = "./LA_{}_seed{}_{}_bs{}_labbs{}_{}labeled_mratio{}".format(args.exp, args.seed, args.model, args.batch_size, args.labeled_bs, args.labelnum, args.mask_ratio)
...
if __name__ == "__main__":
    ...
    def create_dataloader():
        if labelnum == 4:
            train_labset = StrongWeakLAHeart(args.root_path, split='lab4')
            train_unlabset = StrongWeakLAHeart(args.root_path, split='unlab4')
        elif labelnum == 8:
            train_labset = StrongWeakLAHeart(args.root_path, split='lab8')
            train_unlabset = StrongWeakLAHeart(args.root_path, split='unlab8')
        elif labelnum == 16:
            train_labset = StrongWeakLAHeart(args.root_path, split='lab16')
            train_unlabset = StrongWeakLAHeart(args.root_path, split='unlab16')
        else:
            raise NotImplementedError("labelnum should be 4, 8, or 16 for LA dataset.")

        testset = StrongWeakLAHeart(args.root_path, split='test') 
    ...
    for epoch_num in iterator:
        for step, (labeled_batch, unlabeled_batch) in enumerate(zip(trainlab_loader, trainunlab_loader)):
            ...
            strong_batch = torch.cat([torch.zeros_like(strong_batch_u)[:args.labeled_bs], strong_batch_u], dim=0)
            ...
            loss = sup_loss + args.ada_u_weight * unsup_loss + args.ada_u_weight * unsup_loss_mf + as_weight * comp_loss + args.bcp_weight * (loss_l + loss_u)
            ...
            if args.lr_schedule == 'multistep':
                ...
                warmup_iterations = 500 # default 500 now.
            ...
            if iter_num % 200 == 0:
                model.eval()
                dice_sample = test_3d_patch.var_all_case_LA(model, num_classes=num_classes, patch_size=patch_size, stride_xy=18, stride_z=4, data_path=args.root_path) # LA validation 
