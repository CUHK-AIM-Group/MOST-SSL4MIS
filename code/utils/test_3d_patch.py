...
def var_all_case_LA(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, data_path=None):
   
    with open(os.path.join(data_path, 'test.list'), 'r') as f:
        image_list = f.readlines()
    image_list = [os.path.join(data_path, "2018LA_Seg_Training Set", item.replace('\n', '') + "/mri_norm2.h5") for item in image_list]
...
def var_all_case_Pancreas(model, num_classes, patch_size=(96, 96, 96), stride_xy=16, stride_z=4, data_path=None):
   
    with open(os.path.join(data_path, 'test.list'), 'r') as f:
        image_list = f.readlines()
    image_list = [os.path.join(data_path, "data", item.replace('\n', '') + ".h5") for item in image_list]
    loader = tqdm(image_list)
    total_dice = 0.0
    for image_path in loader:
        h5f = h5py.File(image_path, 'r')
...
def var_all_case_Brats(model, num_classes, patch_size=(96, 96, 96), stride_xy=64, stride_z=64, data_path=None):
   
    with open(os.path.join(data_path, 'val.list'), 'r') as f:
        image_list = f.readlines()
    image_list = [os.path.join(data_path, "data", item.replace('\n', '') + ".h5") for item in image_list]
    loader = tqdm(image_list)
